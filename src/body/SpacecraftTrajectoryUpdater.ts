...
export function hermiteSample(pts: TrajectoryPoint[], t: number): [Vector, Vector] | undefined {
  ...
  // const p = Vector.fromV3(a.position)
  // const v = Vector.fromV3(a.velocity)

  return [p, v];
}


// cupdate(bodyObjects3D: Map<string, BodyObject3D>, timestepMs: number, timeMs: number, clock: Clock): Map<string, BodyObject3D> {
//     const t = timeMs +timestepMs;
...
// export function chermiteSample(pts: TrajectoryPoint[], t: number): [Vec3, Vec3] | undefined {
//   const last = pts.length - 1;
//   if (t < pts[0].timeMs || t > pts[last].timeMs) return undefined;
